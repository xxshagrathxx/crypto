...
const App = () => {
  ...
  const currencyPairs = [
    ...
    { symbol: 'SUIUSDT', name: 'SUI (SUI) to Tether (USDT)' },
    { symbol: 'HIGHUSDT', name: 'HIGH (HIGH) to Tether (USDT)' },
  ];
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        const response = await axios.get(
          `https://92a2361e-297e-4946-922f-331771eea325-00-tymkgeatgvdw.kirk.replit.dev/api/market_data?sma_short=${smaShort}&sma_long=${smaLong}&rsi_period=${rsiPeriod}&symbol=${currencyPair}`
        );
    ...
    const fetchPrediction = async () => {
      try {
        const response = await axios.get(
          `https://92a2361e-297e-4946-922f-331771eea325-00-tymkgeatgvdw.kirk.replit.dev/api/predict?symbol=${currencyPair}&${predictionType}=${predictionValue}`
        );
  ...
  const fetchHistoricalMarketCap = async () => {
    try {
      const response = await axios.get(
        `https://92a2361e-297e-4946-922f-331771eea325-00-tymkgeatgvdw.kirk.replit.dev/api/historical_market_cap?symbol=${currencyPair}&days=${daysAgo}&hours=${hoursAgo}&minutes=${minutesAgo}`
      );
      const historicalCap = response.data.market_cap;

      // Set historical market cap
      setHistoricalMarketCap(historicalCap);

      // Calculate and set percentage change
      if (marketData.market_cap && historicalCap) {
        const currentCap = marketData.market_cap;
        const percentageChange = ((currentCap - historicalCap) / historicalCap) * 100;
        setMarketCapChange(percentageChange);
      }

      setError(null);
    } catch (err) {
      setError(err.message || 'An error occurred while fetching historical market cap.');
    }
  };
  ...
  return (
    <div className="container my-5">
      ...
      {/* Historical Market Cap Section */}
      <div className="card mb-4">
        <div className="card-header bg-info text-white text-center">Market Cap Comparison</div>
        <div className="card-body">
          <div className="mb-3">
            <label htmlFor="daysAgo" className="form-label">
              Days Ago:
            </label>
            <input
              type="number"
          ...
          <div className="mb-3">
            <label htmlFor="hoursAgo" className="form-label">
              Hours Ago:
            </label>
            <input
              type="number"
          ...
          <div className="mb-3">
            <label htmlFor="minutesAgo" className="form-label">
              Minutes Ago:
            </label>
            <input
              type="number"
          ...
          <button className="btn btn-primary" onClick={fetchHistoricalMarketCap}>
            Fetch Historical Market Cap
          </button>
          {historicalMarketCap !== null && (
            <div className="mt-3">
              <p>
                <strong>Historical Market Cap:</strong> ${historicalMarketCap.toLocaleString()}
              </p>
              <p>
                <strong>Change:</strong>{' '}
                <span
                  className={`text-${
                    marketCapChange > 0 ? 'success' : marketCapChange < 0 ? 'danger' : 'secondary'
                  } fw-bold`}
                >
                  {marketCapChange > 0 ? '+' : ''}
